...
class SptOpL1A3Controller extends Controller
{
    use RecalculatesL1A7Totals;
    /**
     * Store a newly created L1A3 record.
     */
    ...
    /**
     * Update the specified L1A3 record.
     */
    public function update(Request $request, string $id)
    {
        $record = SptOpL1A3::findOrFail($id);
    ...
    /**
     * Remove the specified L1A3 record(s).
     */
    public function destroy(Request $request)
    {
        $validated = $request->validate([
            'ids' => 'required|array',
            'ids.*' => 'uuid|exists:spt_op_l_1_a3,id',
        ]);

        // Get the spt_op_id before deleting
        $firstRecord = SptOpL1A3::find($validated['ids'][0]);
        $sptOpId = $firstRecord?->spt_op_id;

        SptOpL1A3::whereIn('id', $validated['ids'])->delete();

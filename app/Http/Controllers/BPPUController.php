...
class BPPUController extends Controller
{
    public function index()
    {
        ...
        if (!$activeCourseId) {
            return Inertia::render('BPPU/NotIssued', [
                'bupots' => [],
        ...
        $bupots = Bupot::with('object')
            ...
            ->whereHas('object', fn($q) => $q->where('type', 'bppu'))
    ...
    public function issued()
    {
        ...
        $activeCourseId = session('active_course_id');
        if (!$activeCourseId) {
            return Inertia::render('BPPU/Issued', [
                'bupots' => [],
                'active_course_id' => null,
                'no_active_course' => true,
            ]);
    ...
    public function invalid()
    {
        ...
        $activeCourseId = session('active_course_id');
        if (!$activeCourseId) {
            return Inertia::render('BPPU/Invalid', [
                'bupots' => [],
                'active_course_id' => null,
                'no_active_course' => true,
            ]);
    ...
    public function show($id)
    {
        ...
        $bupot = Bupot::with('object')
            ...
            ->firstOrFail();
        return Inertia::render('BPPU/BPPU', [
            'user' => $user,
            'bupot' => $bupot,
            'active_course_id' => $activeCourseId,
        ]);
    ...
    public function create()
    {
        ...
        $activeCourseId = session('active_course_id');
        if (!$activeCourseId) {
            return Inertia::render('BPPU/FormCreateBPPU', [
                'user' => $user,
                'objects' => [],
                'bupots' => [],
                'active_course_id' => null,
        ...
        $objects = MasterObject::where('type', 'bppu')->get();
        $bupots = Bupot::with('object')
            ->where('user_id', $user->id)
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('bppu.notIssued')->with('success', 'BPPU berhasil dibuat');
        } catch (\Exception $e) {
            return redirect()->route('bppu.notIssued')->with('success', 'Terjadi kesalahan saat membuat BPPU');
        }
    }
    ...
    public function edit($id)
    {
        ...
        $bupot = Bupot::where('id', $id)
            ...
            ->firstOrFail();
        $objects = MasterObject::where('type', 'bppu')->get();
        return Inertia::render('BPPU/FormEditBPPU', [
            'user' => $user,
            'bupot' => $bupot,
            'objects' => $objects,
            'active_course_id' => $activeCourseId,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('bppu.notIssued')->with('success', 'BPPU berhasil diedit');
        } catch (\Exception $e) {
            return redirect()->route('bppu.notIssued')->with('success', 'Terjadi kesalahan saat mengedit BPPU');
        }
    }
    ...
    public function updateStatusMultiple(Request $request)
    {
        ...
        if (!Hash::check($request->password, $user->password)) {
            return redirect()->route('bppu.notIssued')->with('error', 'Password salah!');
        }
        ...
        try {
            ...
            return redirect()->route('bppu.issued')->with('success', 'Status bupot berhasil diperbarui.');
        } catch (\Exception $e) {
            return redirect()->route('bppu.notIssued')->with('error', 'Status bupot gagal diperbarui.');
        }
    }
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('bppu.notIssued')->with('success', 'Bupot berhasil dihapus.');
        } catch (\Exception $e) {
            return redirect()->route('bppu.notIssued')->with('error', 'Gagal menghapus Bupot.');
        }
    }
    ...
    public function downloadPDF($id)
    {
        ...
        return $pdf->stream('bppu_' . $bupot->bupot_number . '_' . $bupot->customer_id . '.pdf');
    }
}

...
<head>
    <title>Bukti Penerimaan Elektronik - Unifikasi</title>
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td class="value">: {{ $spt->sptUnifikasi->total_jumlahpph == 0 ? 'Nihil (0)' : ($spt->sptUnifikasi->total_jumlahpph < 0 ? 'Lebih Bayar (' . number_format(abs($spt->sptUnifikasi->total_jumlahpph), 0, ',', '.') . ')' : 'Kurang Bayar (' . number_format($spt->sptUnifikasi->total_jumlahpph, 0, ',', '.') . ')') }}</td>
        </tr>
        <tr>

...
import type { L12B6Item } from "./types";
...
function FormDialog({ open, onClose, sptBadanId, editData }: FormDialogProps) {
    ...
    useEffect(() => {
        ...
        if (editData) {
            ...
            setDeedNumber(editData.deed_participation_number ?? "");
            setDeedDate(
                editData.deed_participation_date
                    ? editData.deed_participation_date.substring(0, 10)
                    : "",
            );
            setDeedNotary(editData.deed_participation_notary ?? "");
            ...
            setIsCompanyListed(editData.is_company_listed ?? false);
            setStockExchangeName(editData.stock_exchange_name ?? "");
        } else {
            setName("");
            setNpwp("");
            setAddress("");
    ...
    const handleSave = () => {
        ...
        const payload = {
            ...
            deed_participation_number: deedNumber || null,
            deed_participation_date: deedDate || null,
            deed_participation_notary: deedNotary || null,
            investment_value: investmentValue,
            active_period: activePeriod || null,
            is_company_listed: isCompanyListed,
            stock_exchange_name: isCompanyListed
        ...
        if (editData?.id) {
            router.put(
                route("spt.badan.l12b6.update", editData.id),
                payload,
                afterSave,
            );
        } else {
            router.post(route("spt.badan.l12b6.store"), payload, afterSave);
        }
    };
                ...
                <div className="p-4 sm:p-6 space-y-4 overflow-y-auto min-h-0">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-[180px_1fr] items-center gap-4">
                        ...
                        <div className="flex items-center">
                            <Checkbox
                                id="isListed"
                                checked={isCompanyListed}
                                onCheckedChange={(c) => setIsCompanyListed(!!c)}
                            />
                        </div>
                    </div>
                    ...
                    {isCompanyListed && (
                        <div className="grid grid-cols-1 sm:grid-cols-[180px_1fr] items-center gap-4">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setStockExchangeName(e.target.value)
                                }
                            />
                        </div>
                    )}
...
interface SectionL12B6Props {
    data: L12B6Item[];
    sptBadanId: string;
}

export function SectionL12B6({ data, sptBadanId }: SectionL12B6Props) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L12B6Item | null>(null);
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l12b6.destroy"), {
            data: { ids },
            preserveScroll: true,
            onSuccess: () => {
                setSelectedIds([]);
                        ...
                        {data.length === 0 ? (
                            ...
                            data.map((item, idx) => (
                                <TableRow key={item.id ?? idx}>
                                    ...
                                    <TableCell className="border border-gray-200">
                                        {item.deed_participation_number ?? "-"}
                                    </TableCell>
                                    <TableCell className="border border-gray-200 text-center">
                                        {formatDate(
                                            item.deed_participation_date,
                                        )}
                                    </TableCell>
                                    <TableCell className="border border-gray-200">
                                        {item.deed_participation_notary ?? "-"}
                                    </TableCell>
                                    ...
                                    <TableCell className="border border-gray-200 text-center">
                                        {item.is_company_listed
                                            ? "Ya"
                                            : "Tidak"}
                                    </TableCell>
                                    <TableCell className="border border-gray-200">
                                        {item.stock_exchange_name ?? "-"}
                                    </TableCell>
                                </TableRow>
                            ))
                        )}

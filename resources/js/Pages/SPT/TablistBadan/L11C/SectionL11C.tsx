...
import { FormL11CDialog } from "./FormL11CDialog";
import { TableL11C } from "./TableL11C";
import type { L11CItem } from "./types";

interface SectionL11CProps {
    data: L11CItem[];
    sptBadanId: string;
}

export function SectionL11C({ data, sptBadanId }: SectionL11CProps) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L11CItem | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L11CItem) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l11c.destroy"), {
            data: { ids },
            preserveScroll: true,

...
import { FormL11A4ADialog } from "./FormL11A4ADialog";
import { TableL11A4A } from "./TableL11A4A";
import { L11A4AItem } from "./types";

interface SectionL11A4AProps {
    data: L11A4AItem[];
    sptBadanId: string;
}

export function SectionL11A4A({ data, sptBadanId }: SectionL11A4AProps) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L11A4AItem | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L11A4AItem) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l11a4a.destroy"), {
            data: { ids },
            preserveScroll: true,
    ...
    return (
        <div className="space-y-4">
            ...
            />
            <FormL11A4ADialog open={isFormOpen} onClose={() => { setIsFormOpen(false); setEditItem(null); }} sptBadanId={sptBadanId} editData={editItem} />
            <ConfirmDialog

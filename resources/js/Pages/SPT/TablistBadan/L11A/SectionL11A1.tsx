...
import { FormL11A1Dialog } from "./FormL11A1Dialog";
import { TableL11A1 } from "./TableL11A1";
import { L11A1Item } from "./types";

interface SectionL11A1Props {
    data: L11A1Item[];
    sptBadanId: string;
}

export function SectionL11A1({ data, sptBadanId }: SectionL11A1Props) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L11A1Item | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L11A1Item) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l11a1.destroy"), {
            data: { ids },
            preserveScroll: true,
    ...
    return (
        <div className="space-y-4">
            ...
            />
            <FormL11A1Dialog open={isFormOpen} onClose={() => { setIsFormOpen(false); setEditItem(null); }} sptBadanId={sptBadanId} editData={editItem} />
            <ConfirmDialog

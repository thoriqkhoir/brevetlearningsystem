...
export default function TabL1D(props: L1AProps) {
    return (
        <TabL1AComponent
            {...props}
            sectorCode="d"
            a1Categories={A1D}
            heading="REKONSILIASI LAPORAN KEUANGAN - JASA"
        />
    );
}
...
function LegacyTabL1D({
    ...
    const a1Accounts = useMemo(() => {
        ...
        for (const acc of masterAccounts ?? []) {
            const cats = A1D as readonly string[];

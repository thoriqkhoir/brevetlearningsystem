...
import { FormL14Dialog } from "./FormL14Dialog";
import { TableL14 } from "./TableL14";
import type { L14Item } from "./types";

interface SectionL14Props {
    data: L14Item[];
    sptBadanId: string;
}

export function SectionL14({ data, sptBadanId }: SectionL14Props) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L14Item | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L14Item) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l14.destroy"), {
            data: { ids },
            preserveScroll: true,
            onSuccess: () => {
                setSelectedIds([]);

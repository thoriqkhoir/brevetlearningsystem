...
import { FormL11B2ADialog } from "./FormL11B2ADialog";
import { TableL11B2A } from "./TableL11B2A";
import type { L11B2AItem } from "./types";

interface SectionL11B2AProps {
    data: L11B2AItem[];
    sptBadanId: string;
}

export function SectionL11B2A({ data, sptBadanId }: SectionL11B2AProps) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L11B2AItem | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L11B2AItem) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l11b2a.destroy"), {
            data: { ids },
            preserveScroll: true,

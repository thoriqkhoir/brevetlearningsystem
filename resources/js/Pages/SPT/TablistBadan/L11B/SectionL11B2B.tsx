...
import { FormL11B2BDialog } from "./FormL11B2BDialog";
import { TableL11B2B } from "./TableL11B2B";
import type { L11B2BItem } from "./types";

interface SectionL11B2BProps {
    data: L11B2BItem[];
    sptBadanId: string;
}

export function SectionL11B2B({ data, sptBadanId }: SectionL11B2BProps) {
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [editItem, setEditItem] = useState<L11B2BItem | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
    const [deleteId, setDeleteId] = useState<string | null>(null);

    const handleAdd = () => { setEditItem(null); setIsFormOpen(true); };
    const handleEdit = (item: L11B2BItem) => { setEditItem(item); setIsFormOpen(true); };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route("spt.badan.l11b2b.destroy"), {
            data: { ids },
            preserveScroll: true,

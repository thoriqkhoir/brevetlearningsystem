...
const TYPE: "jasa" = "jasa";

export default function TabL3A2({
    user,
    spt,
    sptOpId,
    masterAccounts,
    ...
    // Group A.1 accounts by category (Jasa specific)
    ...
    // Compute summary rows dynamically for JASA
    const computedSummaryRow = (acc: MasterAccount): L3A13A1Item | null => {
        ...
        if (name.includes("laba kotor")) {
            const pendapatan = sumA1RowsByCategory("Pendapatan");
            const bebanLangsung = sumA1RowsByCategory("Beban Langsung");
            return subtractRows(pendapatan, bebanLangsung);
        }
    ...
    const computedSummaryRowFromDraft = (
        ...
        if (name.includes("laba kotor")) {
            return subtractRows(
                sumByCategoryFromDraft("Pendapatan"),
                sumByCategoryFromDraft("Beban Langsung"),
            );
        }

...
export default function ReturDokumenKeluaran({
    returOthers,
    courseResults,
}: {
    returOthers: any[];
    ...
    if (
        returOthers.filter((retur: any) => retur.type === "keluaran").length ===
        0
    ) {
        return (
            <div className="mb-4 rounded-xl bg-sidebar border p-4">
                ...
                <p className="text-gray-500">
                    Belum ada retur dokumen lain keluaran yang dibuat.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="mb-4 rounded-xl bg-sidebar border p-4">
            <h2 className="font-semibold mb-2">Retur Dokumen Lain Keluaran</h2>
                ...
                <table className="min-w-full border text-sm">
                    ...
                    <tbody>
                        {returOthers
                            .filter(
                                (returOther: any) =>
                                    returOther.type === "keluaran"
                            )
                            .map((returOther: any, idx: number) => {
                                const courseResult = courseResults.find(
                                    (result: any) =>
                                        result.other_id === returOther.id
                                );

                                return (
                                    <tr key={returOther.id}>
                                        <td className="border px-2 py-1">
                                            {idx + 1}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.other?.other_no ??
                                                returOther.other?.id ??
                                                "-"}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.retur_number ?? "-"}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.retur_period &&
                                            returOther.retur_year
                                                ? `${returOther.retur_period} ${returOther.retur_year}`
                                                : "-"}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.dpp
                                                ? `Rp ${returOther.dpp.toLocaleString(
                                                      "id-ID"
                                                  )}`
                                                : "-"}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.dpp_lain
                                                ? `Rp ${returOther.dpp_lain.toLocaleString(
                                                      "id-ID"
                                                  )}`
                                                : "-"}
                                        </td>
                                        <td className="border px-2 py-1">
                                            {returOther.ppn
                                                ? `Rp ${returOther.ppn.toLocaleString(
                                                      "id-ID"
                                                  )}`
                                                : "-"}
                                        </td>

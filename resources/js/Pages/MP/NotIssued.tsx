...
export default function NotIssued({ bupots }: any) {
    ...
    const [selectedMp, setSelectedMp] = useState<any[]>([]);
    ...
    const [initialMp, setInitialMp] = useState(bupots);
    ...
    const handleRefresh = () => {
        router.get(
            ...
            {
                onSuccess: (page) => {
                    setInitialMp(page.props.bupots);
                },
            }
        );
    };
    ...
    const handleSelectIds = (ids: string[]) => {
        setSelectedIds(ids);
        const selected = initialMp.filter((bupots: any) =>
            ids.includes(bupots.id)
        );
        setSelectedMp(selected);
    };

    const handleApproveSelected = (selectedIds: string[], password: string) => {
        router.patch(
            route("mp.updateStatusMultiple"),
    ...
    const handleDeleteSelected = (selectedIds: string[], password: string) => {
        router.delete(route("mp.deleteMultiple"), {
    ...
    return (
        <Authenticated>
            ...
            />
            <ConfirmDialog
                title="Terbitkan eBupot MP"
                description="Apakah Anda yakin ingin menerbitkan eBupot MP yang Anda pilih?"
            ...
            />
            <ConfirmDialog
                title="Hapus eBupot MP"
                description="Apakah Anda yakin ingin menghapus eBupot MP yang Anda pilih?"

...
export default function NotIssued({ bupots }: any) {
    ...
    const [selectedBppu, setSelectedBppu] = useState<any[]>([]);
    ...
    const [initialBppu, setInitialBppu] = useState(bupots);
    ...
    const handleRefresh = () => {
        router.get(
            route("bppu.notIssued"),
            {},
            {
                onSuccess: (page) => {
                    setInitialBppu(page.props.bupots);
                },
            }
        );
    };
    ...
    const handleSelectIds = (ids: string[]) => {
        setSelectedIds(ids);
        const selected = initialBppu.filter((bupots: any) =>
            ids.includes(bupots.id)
        );
        setSelectedBppu(selected);
    };

    const handleApproveSelected = (selectedIds: string[], password: string) => {
        router.patch(
            route("bppu.updateStatusMultiple"),
    ...
    const handleDeleteSelected = (selectedIds: string[], password: string) => {
        router.delete(route("bppu.deleteMultiple"), {
    ...
    return (
        <Authenticated>
                    ...
                    <div className="p-5 md:p-8 rounded-xl bg-sidebar border">
                        <div className="flex gap-2 mb-2">
                            ...
                            <Button
                                ...
                                disabled={selectedBppu.length === 0}
                            >
                                <Trash />
                                Hapus
                            </Button>
                            <Button
                                ...
                                disabled={
                                    selectedBppu.length === 0 ||
                                    selectedBppu.some(
                                        (bppu: any) => bppu.status !== "created"
                                    )
                                }
                            >
                                <Send /> Terbitkan
            ...
            />
            <ConfirmDialog
                title="Terbitkan BPPU"
                description="Apakah Anda yakin ingin menerbitkan BPPU yang Anda pilih?"
            ...
            <ConfirmDialog
                title="Hapus BPPU"
                description="Apakah Anda yakin ingin menghapus BPPU yang Anda pilih?"

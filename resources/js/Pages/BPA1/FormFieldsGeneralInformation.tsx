...
export default function FormFieldsGeneralInformation({
    ...
    const [isPopoverGenderOpen, setIsPopoverGenderOpen] = useState(false);
    ...
    const [isPopoverForeignWorkerOpen, setIsPopoverForeignWorkerOpen] = useState(false);
    ...
    const [isForeignWorker, setIsForeignWorker] = useState("");
    ...
    // Di bagian useEffect, tambahkan logika untuk auto-fill saat edit mode
    useEffect(() => {
        if (isEditMode) {
            const customerCountry = form.watch("customer_country");
            if (customerCountry && customerCountry !== "Indonesia") {
                setIsForeignWorker("ya");
            } else if (customerCountry === "Indonesia") {
                setIsForeignWorker("tidak");
            }
        }
    }, [isEditMode, form]);

    // Use the same period options for end_period as well
    // since end_period should be within the same range as start_period
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Pegawai Asing Field - Frontend Only */}
            <div className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
                <label className="md:w-1/4 text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    Pegawai Asing*
                </label>
                <Popover
                    open={isPopoverForeignWorkerOpen}
                    onOpenChange={setIsPopoverForeignWorkerOpen}
                >
                    <PopoverTrigger asChild>
                        <Button
                            variant="outline"
                            ...
                            className={cn(
                                "w-full justify-between",
                                !isForeignWorker && "text-muted-foreground",
                                "whitespace-normal break-words"
                            )}
                            disabled={isEditMode}
                        >
                            <span className="truncate">
                                {isForeignWorker
                                    ? foreignWorkerOptions.find(
                                        (option) => option.value === isForeignWorker
                                    )?.label
                                    : "Pilih Opsi"}
                            </span>
                            <ChevronsUpDown className="opacity-50" />
                    ...
                    <PopoverContent className="w-full p-0">
                        <Command>
                            ...
                            <CommandList>
                                ...
                                <CommandGroup>
                                    {foreignWorkerOptions.map((option) => (
                                        <CommandItem
                                            key={option.value}
                                            onSelect={() => {
                                                setIsForeignWorker(option.value);
                                                
                                                // Auto set negara ke Indonesia jika pegawai asing = tidak
                                                if (option.value === "tidak") {
                                                    form.setValue("customer_country", "Indonesia");
                                                    form.setValue("customer_passport", "");
                                                    form.setValue("customer_address", "");
                                                } else if (option.value === "ya") {
                                                    // Reset negara ke kosong jika pegawai asing = ya
                                                    form.setValue("customer_country", "");
                                                }
                                                
                                                setIsPopoverForeignWorkerOpen(false);
                                            }}
                                        >
                                            {option.label}
                                        </CommandItem>
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
                        <FormLabel className="md:w-1/4">Jabatan*</FormLabel>
                        <FormControl>
                            <Input {...field} disabled={isEditMode} />
                        </FormControl>
                        <FormMessage />
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
                            ...
                            <FormControl>
                                <Input {...field} disabled={isEditMode} placeholder="Masukkan negara" />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    )}
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
                        <FormLabel className="md:w-1/4">Jenis Kelamin*</FormLabel>
                        <Popover
                            open={isPopoverGenderOpen}
                            onOpenChange={setIsPopoverGenderOpen}
                        >
                            <PopoverTrigger asChild>
                                <FormControl>
                                    <Button
                            ...
                            <PopoverContent className="w-full p-0">
                                <Command>
                                    ...
                                    <CommandList>
                                        <CommandEmpty>
                                            Tidak ada jenis kelamin yang tersedia.
                                        </CommandEmpty>
                                        <CommandGroup>
                                            {genders.map((gender) => (
                                                <CommandItem
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem className="flex flex-col gap-1 md:flex-row md:items-center md:gap-4">
                        <FormLabel className="md:w-1/4">Kode Pajak</FormLabel>
                        <FormControl>
                            <Input {...field} disabled />
                        </FormControl>
                        <FormMessage />
